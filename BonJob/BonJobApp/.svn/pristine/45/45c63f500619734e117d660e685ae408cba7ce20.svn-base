...
//#define kJABBER_HOSTNAME @"172.104.8.51"//139.162.164.98"
#define kJABBER_HOSTNAME @"bonjob.co"
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
    ...
    NSString *presenceFromUser = [[presence from] user]; // vishal commented this
    NSString *myUserId=[[NSUserDefaults standardUserDefaults] valueForKey:@"userID"];
    NSArray *foo=[myUserId componentsSeparatedByString:@"@"];
    
    // only testing above 2 lines
    // !commented in if condition by vishal
    if (![presenceFromUser isEqualToString:[foo objectAtIndex:0]])
    {
        ...
        if ([presenceType isEqualToString:@"available"])
        {
            ...
            [dict setValue:[NSString stringWithFormat:@"%@@%@",presenceFromUser,kDefaultChatServer] forKey:@"username"];
            [dict setValue:presenceType forKey:@"status"];
            [[NSNotificationCenter defaultCenter]postNotificationName:@"statusChanged" object:nil userInfo:dict];
...
//vishal implemented this to chek buddy status
-(void)checkBuddyPresence
{
    
//    NSXMLElement *presence = [NSXMLElement elementWithName:@"presence"];
//    
//    [presence addAttributeWithName:@"to" stringValue:@""];
//    
//    
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
    ...
    if([[message elementForName:@"body"]stringValue] != nil){ //&& userData){
       ...
       __strong NSString *to = [[message attributeForName:@"to"] stringValue];
     ...
     //   NSDate*date=[NSString getCurrentDateFromString:[NSString getCurrentTime]];
      //  NSString* time=[[Alert getDateFormatWithString:GET_FORMAT_TYPE] stringFromDate:date];
        ...
        // vishal added for android to ios chat issue
        if(timeStamp==nil || timeStamp.length==0)
        {
            NSDate*date=[NSString getCurrentDateFromString:[NSString getCurrentTime]];
            NSString* time=[[Alert getDateFormatWithString:GET_FORMAT_TYPE] stringFromDate:date];
            _interval=[time doubleValue];
        ...
        [dateFormatter setDateFormat:GET_FORMAT_TYPE];
...
-(NSString*)getRoomId:(NSString*)resToUser
{
    ...
    strRoomId = [strRoomId stringByAppendingString:@"bonjob.co"];
   return strRoomId;
}
