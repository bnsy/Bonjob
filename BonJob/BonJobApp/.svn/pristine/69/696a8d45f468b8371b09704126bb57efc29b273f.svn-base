...
{
    NSLog(@"%@",self.products[0]);
    if ([self validate])
    {
        [currentTextfield resignFirstResponder];
        if ([[self.planDict valueForKey:@"subscription_id"] intValue]==1)
        {
        SKProduct *prod = self.products[0];
            NSLog(@"3%@3",prod.productIdentifier);
            if ([prod.productIdentifier isEqualToString:@"com.ludovic.mensuel"]) {
        ...
        {
            SKProduct *prod = self.products[0];
            NSLog(@"%@",prod.productIdentifier);
            if ([prod.productIdentifier isEqualToString:@"com.ludovic.annuel"]) {
                SKPayment *payment = [SKPayment paymentWithProduct:self.products[0]];
                [[SKPaymentQueue defaultQueue] addPayment:payment];
            }
            else
...
-(void)payAMount:(NSString *)tokenID
{
    ...
    if ([self validate])
    {
        ...
        [params setValue:_txtVill.text forKey:@"location"];
