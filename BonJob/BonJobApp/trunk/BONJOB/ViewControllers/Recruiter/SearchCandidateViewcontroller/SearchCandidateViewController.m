...
-(BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField.text.length > 0) {
        ...
        [_tblCandidates scrollRectToVisible:CGRectMake(0, 0, 1, 1) animated:YES];
    }
    else{
        pageNumber = 0;
        [self getDefaultCandidate];
...
-(void)getDefaultCandidate
{
    ...
    if (APPDELEGATE.isFilterRecruiter == YES) {
         NSMutableArray *temp =[[NSMutableArray alloc]init];
        for (StatusModel * obj  in APPDELEGATE.arraySelStatusRecruiter) {
            [temp addObject:obj.seeker_current_status_id];
        }
        [params setValue:temp forKey:@"current_status"];
        temp =[[NSMutableArray alloc]init];
        
        for (LevelOfEducationModel * obj  in APPDELEGATE.arraySelEducationRecruiter) {
            [temp addObject:obj.education_id];
        }
        [params setValue:temp forKey:@"education_level"];
        temp =[[NSMutableArray alloc]init];
        
        for (MobilityModel * obj  in APPDELEGATE.arraySelMobilityRecruiter) {
            [temp addObject:obj.mobility_id];
        }
        [params setValue:temp forKey:@"mobility"];
        temp =[[NSMutableArray alloc]init];
        
        for (SkillsModel * obj  in APPDELEGATE.arraySelSkillsRecruiter) {
        ...
        for (LevelOfLanguageModel * obj  in APPDELEGATE.arraySelLanguageRecruiter) {
            [temp addObject:obj.job_language_id];
        }
        [params setValue:temp forKey:@"language"];
        temp =[[NSMutableArray alloc]init];
        
        for (ExperienceModel * obj  in APPDELEGATE.arraySelExperienceRecruiter) {
