...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [self.view addKeyboardPanningWithActionHandler:^(CGRect keyboardFrameInView, BOOL opening, BOOL closing) {
        ...
        {
            CGRect toolBarFrame = inputbar.frame;
            toolBarFrame.origin.y =self.view.frame.size.height- 90+ toolBarFrame.size.height;
            inputbar.frame = toolBarFrame;
...
-(void)InitialSetup
{
    UIButton *btn=[[UIButton alloc]initWithFrame:CGRectMake(self.view.frame.size.width-40, 20, 40, 40)];
    [btn setBackgroundImage:[UIImage imageNamed:@"default_photo_deactive.png"] forState:UIControlStateNormal];
...
-(void)setInputbar
{
    self.inputbar.placeholder = NSLocalizedString(@"New Message", @"");
    self.inputbar.delegate = self;
    //self.inputbar.leftButtonImage = [UIImage imageNamed:@"share"];
    self.inputbar.rightButtonText = NSLocalizedString(@"SEND", @"");
    self.inputbar.rightButtonTextColor=TitleColor;
    self.inputbar.rightButtonTextColor = TitleColor;
}
-(void)setTableView
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row==0)
    {
        static NSString *CellIdentifier = @"ChatFixedCell";
        ChatFixedCell *cell = (ChatFixedCell*)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!cell)
        {
            cell = [[ChatFixedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
